...
public class PredicatePushdownLogicalPlanVisitor implements LogicalPlanVisitor {
    ...
    public void visit(LogicalOpRecursive logicalOpRecursive) {
        ...
        if (logicalOpRecursive.hasFirstFilter()) {
            // izquierdo esta filtrado y el derecho no, entonces el conjunto completo tiene
            // que estar en el hijo derecho
            ...
            // derecho esta filtrado y el izquierdo no, entonces el conjunto completo tiene
            // que estar en el hijo izquierdo
            logicalOpRecursive.getRightChild().acceptVisitor(this);
            logicalOpRecursive.setFilters(false, false);
            logicalOpRecursive.getLeftChild().acceptVisitor(this);
        }
        ...
        if (otherSelection != null) {
            ...
            stack.push(new LogicalOpRecursive(stack.pop(), stack.pop(), logicalOpRecursive.hasLastFilter(),
    ...
    public void visit(LogicalOpSelectionByNegatedLabel logicalOpSelectionByNegatedLabel) {
        if (!stack.isEmpty() && stack.peek() instanceof LogicalOpSelection) {
            ...
            if (selection.getChild() != null) {
                ...
                stack.push(new LogicalOpSelection(logicalOpSelectionByNegatedLabel, selection.getCondition()));
                return;
            }
        }
        stack.push(logicalOpSelectionByNegatedLabel);
    }
